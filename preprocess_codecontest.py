...
def create_resplit_index():
    original_train_examples = read_jsonl_file("data/codecontest_train.jsonl")
    original_dev_examples = read_jsonl_file("data/codecontest_valid.jsonl")
    original_test_examples = read_jsonl_file("data/codecontest_test.jsonl")

    text_to_example_idx = {x["description"].strip(): i for i, x in enumerate(original_train_examples)}
    text_to_example_idx = {x["description"].strip(): i for i, x in enumerate(original_dev_examples)}
    text_to_example_idx = {x["description"].strip(): i for i, x in enumerate(original_test_examples)}
